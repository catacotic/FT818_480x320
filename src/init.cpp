...
void show_credits(){
  ...
  tft.drawRect(3,3,476,314,TFT_WHITE); 
  tft.setFreeFont(&FreeSansBold9pt7b); 
  tft.setTextColor(TFT_WHITE,TFT_BLACK);
  tft.drawCentreString("Yaesu FT818 Control CAT by:", 240,60,GFXFF);
  tft.setFreeFont(&FreeSansBold12pt7b); 
  tft.setTextColor(TFT_YELLOW,TFT_BLACK);
  tft.drawCentreString("TACOTIC - EB3AQW", 240,95,GFXFF);
  tft.setFreeFont(&FreeSansBold9pt7b); 
  tft.setTextColor(TFT_YELLOW,TFT_BLACK);
  tft.drawCentreString("firmware V3.1" , 240,125,GFXFF);
  tft.setTextColor(TFT_SILVER,TFT_BLACK);
 
  tft.drawCentreString("Thanks for libraries and docs:", 240,200,GFXFF);
  tft.drawCentreString("Touchscreen by Paul Stoffregen (2021)", 240,220,GFXFF);
  tft.drawCentreString("FT-817-TouchDisplay by Peter Jonas (2021)", 240,240,GFXFF);
   tft.drawCentreString("KA7OEI's blog and FT-817 pages", 240,260,GFXFF);
 
  tft.drawCentreString("www.tacotic.com  -  Public Licence (2025)", 240,290,GFXFF);
...
void init_all() {
  ...
  if (POWER_ON){
  tft.fillRect(20,170,450,42,TFT_BLACK); // 480 x 320
  get_vfo();
  get_band();
  get_mode();
  get_steps();
...
void draw_band(){
  tft.fillRect(155,173,80,37,TFT_BLACK); // 480x320
  tft.setFreeFont(&FreeSansBold12pt7b); 
  tft.setTextColor(TFT_CYAN,TFT_BLACK);
  tft.setTextDatum(TL_DATUM);
  tft.drawString(bands[current_band],157,185, GFXFF);  // 480x320
  draw_antena();
...
void get_mode() {
  ...
  if( current_mode != res || first_run ) {
    ...
    update_menu(1);
    draw_steps(); // if mode change force read and update steps
  }
