...
void release_keys();
void push_tunner(void);  // Send signal to activate ATU100 external tunner

// Analog read functions  
bool inputFTpw();        // Read analog pin for FT818 is on/off (FT818 analog band output less of 0,10 V indicate FT818 TX is power off )
String inputbatvolts();  // Read analog pin for system battery analog volts
bool inputchrvolts();    // Read analog pin for system USB connect to external source 
void start_power();      // initcialize all power variables
void control_power();    // Control of power parámeters 
void read_SYSpw();       // Read system power parameters (USB & BAT) 
void show_FTpw();        // Display icon (green/reed) of FT818 power status
void show_SYSpw();      // Display bat volts, charge bat steps and external source input


extern void calibrate_touch();
extern void text_show_bar();
extern void plotNeedle(int, byte);
extern void drawSMeter();
extern void show_bar(uint8_t, byte);
extern void init_all(void);
extern bool get_vfo(void);
extern void draw_band(void);
extern void get_band(void);
extern void get_mode(void);
extern void get_steps(void);
extern void draw_steps(void);
extern void get_ipo(void);
extern void get_att(void);
extern void get_nb(void);
extern void get_antenna(void);
extern void draw_antena(void);
...
int old_sm=0;
uint8_t old_mode=0;
uint8_t current_mode=0;      // that all the get-calls are done once in the setup()
...
bool POWER_ON = false; // Power on/off swich
