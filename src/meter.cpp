...
void plotNeedle(int value, byte ms_delay)
{
  ...
  do {
    ...
    tft.drawCentreString("S", M_SIZE*120, M_SIZE*70+YOFFSET, GFXFF);
...
void drawSMeter()
{
  ...
  // Draw ticks every 5 degrees from -50 to +50 degrees (100 deg. FSD swing)
   for (int i = -49; i < 50; i += 7) {
    ...
    // Coodinates of tick to draw  320x240
 ...
 // Check if labels should be drawn, with position tweaks
   if (i % 7 == 0) {
      ...
      y0 = sy * (M_SIZE*100 + tl) + M_SIZE*140;
       switch (i / 7) {
        case -7: tft.drawCentreString("0", x0+6, y0-15+YOFFSET, 2); break;
        case -6: tft.drawCentreString("1", x0+5, y0 -15+YOFFSET, 2); break;
        case -4: tft.drawCentreString("3", x0+3, y0 -14+YOFFSET , 2); break;
        case -2: tft.drawCentreString("5", x0+2, y0 -15+YOFFSET, 2); break;
        case 0:  tft.drawCentreString("7", x0, y0 -16+YOFFSET, 2); break;
        case 2:  tft.drawCentreString("9", x0-1, y0 -17+YOFFSET, 2); break;
        case 4:  tft.drawCentreString("+20", x0+6, y0 -16+YOFFSET, 2); break;
        case 6:  tft.drawCentreString("+40", x0+4, y0 -11+YOFFSET , 2); break;
      }
    }
...
void text_show_bar(){
  ...
  tft.drawString("PWR",240,16,GFXFF); // 480x320 (262)
  tft.drawString("SWR",240,43,GFXFF); 
  tft.drawString("ALC",240,69,GFXFF);
  tft.drawString("MOD",240,96,GFXFF); 
}
...
void show_bar(uint8_t value, byte type) {
  ...
  switch(type) {
    ...
    for( uint8_t i=0;i<10;i++) 
        tft.fillRect(295+i*16,19,12,13,TFT_DARKERGREY); // 480x320 
    if(!value) return;
    ...
    for( uint8_t i=1;i<=value;i++) {
      ...
      tft.fillRect(279+i*16,19,12,13,color); // 480x320
    }
    break;

    case 1:
    for( uint8_t i=0;i<10;i++) 
      tft.fillRect(295+i*16,45,12,13,TFT_DARKERGREY); // 480x320
    ...
    for( uint8_t i=1;i<=value;i++) {
     ...
     tft.fillRect(279+i*16,45,12,13,color); // 480x320
    }
    break;

    case 2:
    for( uint8_t i=0;i<10;i++) 
       tft.fillRect(295+i*16,72,12,13,TFT_DARKERGREY); // 480x320
    if(!value) return;
    ...
    tft.fillRect(279+i*16,72,12,13,color); // 480x320
    ...
    case 3:
    for( uint8_t i=0;i<10;i++) 
       tft.fillRect(295+i*16,99,12,13,TFT_DARKERGREY); // 480x320
    ...
    for( uint8_t i=1;i<=value;i++) {
      ...
      tft.fillRect(279+i*16,99,12,13,color); // 480x320
    }
    break;
  }
